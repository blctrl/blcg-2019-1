...
static int set_status(int card, int signal)
{
         ...
         p = epicsStrtok_r(NULL, ",", &tok_save), index++)
    {
        ...
        case 1:         /* encoder status */
            ...
            status.Bits.RA_HOME =      (ax_stat->home == 'H')      ? 1 : 0;  /*ADD FOR ATHM = 1, WHEN HOME SWITCH IS ACTIVE zzh*/
            break;
...
/* add for xxxRecord device support 190903*/
    	#include "drvXxx.h"
	int blc_send_mess(int n,char *mess)
	/* n: card number
	   mess: BH8~BH11 or BL8~BL11 */
	{
	send_mess(n,mess,0);
	return 0;
...
/* add for read athm status 190904*/
	int blc_home_state(int card,int axis,int status,char *buf)
	{
		int n;
		send_recv_mess(1, "QA", (char *) MAXv_axis[axis], buf, n);
		printf("%c\n",buf[3]);
		if(buf[3]=='H') status= 1;
		if(buf[3]=='N') status= 0;
		if((buf[3]!='N')&&(buf[3]!='H')) status= 2;
		printf("%d\n",status);
		return status;
	}
